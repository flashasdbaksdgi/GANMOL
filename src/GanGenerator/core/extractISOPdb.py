...
# extract these grid points from the pdb file
...
if len(sys.argv) > 3:
    ...
    sys.exit(
        " ERROR : please provide the name of the dx file, the isovalue and the outputname :\n python extractISOPdb.py path/my_dx_file.dx outputname.pdb isovalue"
    )
...
print(
    "Reading the grid. Depending on the number of data points you have this might take a while...."
)
...
for count in range(n_entries // 3):
    ...
    for i in range(3):
        if (iso_value < 0 and float(c[i]) < iso_value) or (
            iso_value > 0 and float(c[i]) > iso_value
        ):
            path.write(
