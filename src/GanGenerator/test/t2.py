...
def read_xyz(fnm):
    ...
    for i in range(n):
        l = f.readline().split()
        sym.append(int(l[1]))
        coord.append(map(lambda x: float(x) / 0.529177209, l[2:5]))
        grad.append(map(float, l[5:8]))

    return sym, N.array(coord), N.array(grad)


symbols, coordinates, gradients = read_xyz(
    "/mnt/c/Users/Takshan/Desktop/PhD/LOCAL/lab09/DEV/GAN/data/test/6nzp_protein.xyz"
)
nat = len(symbols)
