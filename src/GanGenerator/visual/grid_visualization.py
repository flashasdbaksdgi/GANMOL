...
def parse_args(argv=None):
    parser = argparse.ArgumentParser(
        description="Script for generating the jobs that need to be run for visualization. Generates types files & a text file that needs to be run. Can make a DX file for visualization",
    ...
    parser.add_argument(
        ...
        help="File containing Receptor atom types of your modelfile (1 per line)\n",
    )
    parser.add_argument(
        "-l",
        "--ligatoms",
        ...
        required=True,
        default=argparse.SUPPRESS,
        help="File containing Ligand atom types of your modelfile (1 per line)\n",
    )
    parser.add_argument(
        "-o",
        "--outname",
        type=str,
        default="grid_predicts.txt",
        help="File containing commands to be evaluated to predict grid points. Note: Requires GNINASCRIPTSDIR to be a set environment variable.\n",
    )
    parser.add_argument(
        "-t",
        "--typesroot",
        type=str,
        default="types/",
        help="Root folder for gninatypes data generated from script.\n",
    )
    parser.add_argument(
        "-m",
        "--model",
        ...
        required=True,
        default=argparse.SUPPRESS,
        help="Model file that predictions will be made with. Must end in .model\n",
    )
    parser.add_argument(
        "-w",
        "--weights",
        ...
        required=True,
        default=argparse.SUPPRESS,
        help="Weights for the model file that the predictions will be made with.\n",
    )
    parser.add_argument(
        "-p",
        "--test_pdb",
        type=str,
        default="gly_gly_gly.pdb",
        help="pdbfile of receptor, centered at the origin for visualization\n",
    )
    parser.add_argument(
        "-c",
        "--cube_length",
        type=float,
        default=24.0,
        help="Width of cube for grid box of points. Defaults are reasonable\n",
    )
    parser.add_argument(
        "-n",
        "--num_points",
        type=int,
        default=20,
        help="Number of points per half of the box (ex 20 means there will be 39x39x39 points total). Defaults are reasonable.\n",
    )
    parser.add_argument(
        "--make_dx",
        action="store_true",
        default=False,
        help="Flag to make dx files from the data. Assumes job(s) have completed.\n",
    )
    parser.add_argument(
        "-d",
        "--dataroot",
        type=str,
        default="data/",
        help="Root folder of data resulting from output\n",
    )
    ...
    for x in val_range:
        for y in val_range:
            for z in val_range:
                ...
                with open(
                    root + atom + "/" + atom + "_" + str(counter) + ".gninatypes", "wb"
                ) as f:
                    f.write(pos)
                    f.write(identity)
                counter += 1
